...
> All persistence mechanism checks are in the dedicated skill:
> **`@~/.claude/skills/linux-persistence/SKILL.md`**
>
> Covers: cron, systemd services/timers, SysV init / rc.local, LD_PRELOAD,
> kernel modules, shell RC files, SSH authorized keys, SSH client config,
> SUID/SGID binaries, APT hooks, DNF/Yum plugins, PAM backdoors, D-Bus
> services, NetworkManager dispatcher scripts, Python .pth files, git hooks.
...
Pattern checks find artifacts by **location and content**; a filesystem timeline or Plaso
supertimeline adds **time-based discovery**. These are complementary, not overlapping.

> **Timeline generation:** `@~/.claude/skills/sleuthkit/SKILL.md` (bodyfile/mactime) and
> `@~/.claude/skills/plaso-timeline/SKILL.md` (Plaso supertimeline).

| Format | Source | Key fields |
|--------|--------|------------|
| **bodyfile** | `fls -r -m / <image>` | Pipe-delimited; `$10`=ctime epoch, `$9`=mtime, `$2`=path |
| **mactime -y** | `mactime -y -b bodyfile.txt` | ISO 8601 CSV — easiest to filter |
| **l2tcsv** | `psort.py -o l2tcsv plaso.sqlite` | `$1`=datetime, `$3`=source; all log+fs sources |

```bash
# Identify format
head -2 ./analysis/timeline.*
# bodyfile: pipe-delimited, first field = 32-char MD5 or "0"
# l2tcsv:   header starts "datetime,timestamp_desc,source,..."
```

### Time-Window Sweep

Set T1/T2 from auth log or audit timestamps. Use **ctime** (`$10` bodyfile) — resists backdating.

```bash
T1=1705276800; T2=1705363199   # date -d "2024-01-15 02:00:00 UTC" +%s
# bodyfile
awk -F'|' -v t1=$T1 -v t2=$T2 '$10>=t1 && $10<=t2 {printf "%s\t%s\n",$10,$2}' \
  ./analysis/bodyfile.txt | sort -n | tee ./exports/timeline_window.txt
# l2tcsv / dynamic — all sources
...
Strip `/mnt/linux_mount` prefix — bodyfile and Plaso paths are image-root-relative.

```bash
{
  grep -oP '"\K/[^"]+' ./exports/apt_hooks_suspicious.txt 2>/dev/null
  cat ./exports/package_manager_plugins_suspicious.txt ./exports/new_executables.txt 2>/dev/null
...
while IFS= read -r path; do
  grep -hF "$path" ./analysis/timeline.* 2>/dev/null
done < /tmp/ioc_paths.txt | tee ./exports/timeline_ioc_hits.txt
```

### Cluster Pivot

Taking the ctime of a confirmed malicious file and finding everything within ±60 s
routinely surfaces implants in unusual locations that pattern checks missed.

```bash
# bodyfile: fields $1=MD5 $2=path $8=atime $9=mtime $10=ctime $11=crtime
PIVOT_CTIME=$(awk -F'|' '/\/root\/.backup\/sshd/{print $10; exit}' ./analysis/bodyfile.txt)
awk -F'|' -v t=$PIVOT_CTIME '$10>=t-60 && $10<=t+60 {printf "%s\t%s\n",$10,$2}' \
  ./analysis/bodyfile.txt | sort -n | tee ./exports/timeline_cluster.txt
# Plaso: same-minute window surfaces correlated log events (SSH logins, commands, etc.)
grep "^2024-01-15T02:13" ./analysis/plaso_supertimeline.csv | tee ./exports/timeline_cluster.txt
```

### Backdating Detection

`touch -t` forges mtime/atime but **cannot** change ctime. Requires bodyfile.

```bash
T1=1705276800; T2=1705363199
