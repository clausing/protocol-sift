...
# Includes /dev/. to catch hidden paths under /dev (e.g. /dev/... /dev/.sneaky)
grep -hE \
  "(/tmp/|/dev/shm|/dev/\.|/var/tmp/|\$\(|base64|wget|curl|nc |ncat |bash -i|python.*-c|perl.*-e|/dev/tcp)" \
  $(cat ./exports/gitconfig_files.txt) 2>/dev/null | \
  tee ./exports/gitconfig_suspicious.txt
```

> **Payload location note:** The gitconfig key is persistent (on disk, recoverable in
> offline forensics). Payload binaries dropped to `/dev/shm`, `/run`, or `/tmp` (tmpfs)
> do not survive reboots and are absent from disk images — see the Volatile Data section
> in `linux-artifacts/SKILL.md` for live capture and auditd/memory pivot commands.

```bash
# Hidden files and directories under /dev on the disk image
# devtmpfs is mounted over /dev at boot on live systems, masking anything
# placed on the underlying filesystem. In offline analysis these ARE visible.
# Attackers use names like /dev/... (three dots) or /dev/.hidden to exploit this.
find /mnt/linux_mount/dev -maxdepth 3 -name ".*" \
  2>/dev/null | tee ./exports/dev_hidden.txt

# Executable files directly in /dev — unusual on any clean system
